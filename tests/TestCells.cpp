...
TEST(ParticleContainer, forAllPairsInSameCell){
    ...
    particles.emplace_back(Eigen::Vector3d{0.1,0.1,0.1}, Eigen::Vector3d{0.,0.,0.}, 1.0, 0);
    particles.emplace_back(Eigen::Vector3d{0.2,0.2, 0.2}, Eigen::Vector3d{0.,0.,0.}, 1.0, 0);
    ...
    pc.forAllPairsInSameCell([](Particle& p1, Particle& p2){
        //beware: these outputs include the transformed coordinates. so  you need to subtra
        ASSERT_FALSE(p1.getX() == p2.getX())<<"Particle at " << p1.getX()  <<" interacted with itself\n";
        ASSERT_TRUE(p1.getType() == p2.getType())<<"Particle " << p1.getX() << " and " << p2.getX() << "are not in the same cell, but they interacted\n";
    });
}
