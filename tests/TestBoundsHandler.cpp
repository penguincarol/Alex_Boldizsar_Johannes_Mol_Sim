...
TEST(BoundsHandler, outflow){
    std::array<bool, 2> ompFlags{true, false};

    for(auto ompFlag : ompFlags) {
        ...
        sim::physics::force::ForceHandler forceHandler{sim::physics::force::lennardJones, true, false, false, false,
                                                       false, 0, 0, 0, 0, 1, 0.1, 1, 1, pc};
...
TEST(BoundsHandler, reflecting){
    ...
    for(auto ompFlag : ompFlags) {
        ...
        pc.forAllParticles([&](Particle &p) {
            EXPECT_EQ(p.getF().norm(), 0.)
                                << "Particle experiences a force even before the boundary condition could be applied";
        });
        bh.operator()();
        ...
        ASSERT_TRUE(std::abs((p1.getX() - p1Copy.getX()).norm()) < p1.getX().norm() * 0.000001)
                                    << "Applying boundary condition did not result in the expected applied force";
