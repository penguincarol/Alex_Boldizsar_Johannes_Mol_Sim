...
namespace sim::physics::force {
    ...
    static void fastPairFunction(vec4d_t &force,
        ...
        d0 = x[4*indexI + 0] - x[4*indexJ + 0];
        d1 = x[4*indexI + 1] - x[4*indexJ + 1];
        d2 = x[4*indexI + 2] - x[4*indexJ + 2];
        s = m[indexI] * m[indexJ] * std::pow(1 / std::sqrt(d0 * d0 + d1 * d1 + d2 * d2), 3);
        df0 = d0 * s;
        df1 = d1 * s;
        df2 = d2 * s;
        force[4*indexI + 0] -= df0;
        force[4*indexI + 1] -= df1;
        force[4*indexI + 2] -= df2;
        force[4*indexJ + 0] += df0;
        force[4*indexJ + 1] += df1;
        force[4*indexJ + 2] += df2;
    }
    ...
    static void fastForeAlt(vec4d_t &force,
                            vec4d_t &x,
                            std::vector<double> &eps,
                            std::vector<double> &sig,
                            std::vector<double> &m,
    ...
    static std::array<double,3> fastForceRet(vec4d_t &force,
                                             vec4d_t &x,
                                             std::vector<double> &eps,
                                             std::vector<double> &sig,
                                             std::vector<double> &m,
